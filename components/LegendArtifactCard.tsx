...
export default function LegendArtifactCard({ artifact }: LegendArtifactCardProps) {
  ...
  const isGiftInvite = artifact.mode === 'gift-invite';
  ...
  return (
    ...
    >
      <div aria-hidden="true" style={{ position: 'absolute', inset: 14, border: '3px double rgba(23,23,23,0.72)' }} />
      <div aria-hidden="true" style={{ position: 'absolute', inset: 28, border: '1px dashed rgba(23,23,23,0.35)' }} />
      <div
        aria-hidden="true"
        style={{
          position: 'absolute',
      ...
      <div style={{ position: 'relative', zIndex: 1, height: '100%', display: 'flex', flexDirection: 'column' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', alignItems: 'flex-start' }}>
          <div>
            ...
            <h1 style={{ margin: '0.35rem 0 0', fontSize: 'clamp(1.25rem, 4vw, 2.45rem)', fontWeight: 700, letterSpacing: '0.08em' }}>
              {isGiftInvite ? 'GIFT ACCESS PASS' : 'PERSONAL MOMENT PASS'}
            </h1>
          </div>
        ...
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'center', gap: '1.5rem' }}>
          <div>
            <div style={{ fontSize: '0.72rem', letterSpacing: '0.18em', textTransform: 'uppercase', opacity: 0.72 }}>
              {isGiftInvite ? 'Access prepared for' : 'Issued to'}
            </div>
            <div style={{ marginTop: '0.35rem', fontSize: 'clamp(1.12rem, 4vw, 2.1rem)', borderBottom: '1px solid #171717', paddingBottom: '0.35rem' }}>
              {artifact.subjectName || 'Personal Holder'}
            </div>

            <div style={{ marginTop: '1.35rem', fontSize: '0.72rem', letterSpacing: '0.18em', textTransform: 'uppercase', opacity: 0.72 }}>
              {isGiftInvite ? 'Gift issued by' : 'Moment authority'}
            </div>
            <div style={{ marginTop: '0.35rem', fontSize: 'clamp(1rem, 3vw, 1.55rem)' }}>
              {artifact.issuedBy || 'Self-claimed moment'}
            </div>
          </div>
        ...
        <footer style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', alignItems: 'flex-end', fontSize: '0.7rem' }}>
          <div style={{ maxWidth: '70%', lineHeight: 1.45 }}>
            {artifact.note || artifact.legend.logic}
          </div>
